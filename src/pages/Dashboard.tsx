...
export function DashboardContent() {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="show"
      className="w-full min-h-screen bg-background text-foreground font-sans p-6 md:p-10"
    >
      <div className="mx-auto w-full max-w-[1600px] space-y-8">
        
        {/* ═══════════ HEADER ═══════════ */}
        <motion.div variants={itemVariants} className="flex flex-col gap-2">
          <h1 className="text-3xl md:text-4xl font-bold text-foreground dark:text-zinc-100">
            Welcome back, {userFirstName}
          </h1>
          <p className="text-sm text-muted-foreground dark:text-zinc-400 max-w-2xl">
            Here's what's happening with your studio today. Track your projects, manage your team, and stay on top of your production schedule.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <motion.div variants={itemVariants}>
            ...
            >
              ...
              <div className="text-3xl font-semibold text-foreground tracking-tight mb-2">
              ...
              <div className="mt-3 text-[11px] text-muted-foreground font-medium flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <motion.div variants={itemVariants} className="lg:col-span-8">
            <div className="bg-card border border-border rounded-[24px] p-6 h-full flex flex-col relative overflow-hidden">
              <div className="flex justify-between items-start z-10 mb-8">
                <div>
                  <h3 className="text-foreground font-semibold text-lg flex items-center gap-2">
                    Financial Overview <span className="w-2 h-2 rounded-full bg-foreground shadow-[0_0_8px_currentColor]"></span>
                  </h3>
                  <p className="text-muted-foreground text-xs mt-1">Comparison to previous periods</p>
                </div>
                <div className="flex items-center bg-muted border border-border rounded-lg p-1">
                  <button
                    ...
                    className={`px-4 py-1.5 text-xs font-medium rounded-md transition-all ${
                      isBalanceToggled ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    Net Balance
                  </button>
              ...
              <div className="flex-1 relative w-full min-h-[300px] rounded-xl border border-border/40 bg-muted/30 overflow-hidden flex items-center justify-center">
                {/* Grid Lines */}
                <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px]"></div>
                
                {/* Glow Effects */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-px h-full bg-gradient-to-b from-transparent via-foreground/50 to-transparent shadow-[0_0_15px_currentColor]"></div>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-px w-full bg-gradient-to-r from-transparent via-foreground/50 to-transparent shadow-[0_0_15px_currentColor]"></div>
                
                {/* Center Node */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-foreground shadow-[0_0_20px_4px_currentColor]"></div>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-foreground/20 rounded-full blur-[60px] pointer-events-none"></div>
                ...
                <motion.div 
                  ...
                  className="absolute top-1/2 left-1/2 translate-x-4 translate-y-4 bg-muted/80 backdrop-blur-md border border-border rounded-xl p-3 shadow-xl dark:bg-[#1a1a1c]/80"
                >
                  <p className="text-muted-foreground text-xs mb-1">Current Total</p>
                  <p className="text-foreground font-semibold text-sm">
                    $ {displayedFinancial.toLocaleString()}
                  </p>
                  <p className="text-muted-foreground text-[10px] mt-1">+ Active Cycle</p>
                </motion.div>
          ...
          <motion.div variants={itemVariants} className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button className="flex items-center gap-1 bg-muted border border-border rounded-md px-2.5 py-1 text-xs text-muted-foreground hover:text-foreground">
                  Today <ChevronDown className="w-3 h-3" />
                </button>
              </div>
              <p className="text-muted-foreground text-xs mb-3">The sum of all operations</p>
              <div className="text-4xl font-bold text-foreground tracking-tight flex items-baseline gap-2">
                $ {netBalance >= 1000 ? (netBalance / 1000).toFixed(2) + 'k' : netBalance} <span className="text-sm font-medium text-muted-foreground">USD</span>
              </div>
            </div>

            {/* Bottom Widget: Smooth Area Chart representation */}
            <div className="bg-card border border-border rounded-[24px] p-5 flex-1 relative overflow-hidden flex flex-col">
              <div className="flex items-center gap-2 mb-6">
                <div className="w-1.5 h-4 bg-foreground rounded-full"></div>
                <h4 className="text-foreground text-sm font-medium">Details Observation</h4>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <p className="text-muted-foreground text-[10px] uppercase tracking-wider mb-1">Events</p>
                  <p className="text-xl font-semibold text-foreground">{scheduleEvents.length}</p>
                  <p className="text-[10px] text-muted-foreground mt-1 flex items-center bg-muted w-fit px-1.5 py-0.5 rounded">
                    <TrendingDown className="w-2.5 h-2.5 mr-1" /> MTD
                  </p>
                </div>
                <div>
                  <p className="text-muted-foreground text-[10px] uppercase tracking-wider mb-1">Income</p>
                  <p className="text-xl font-semibold text-foreground">{totalIncome >= 1000 ? (totalIncome / 1000).toFixed(0) + 'k' : totalIncome}</p>
                  <p className="text-[10px] text-muted-foreground mt-1 flex items-center bg-muted w-fit px-1.5 py-0.5 rounded">
              ...
              <div className="mt-auto h-24 relative w-full">
                <svg viewBox="0 0 200 60" className="w-full h-full overflow-visible preserve-3d">
                  ...
                  />
                  {/* Tooltip point */}
                  <circle cx="150" cy="15" r="3" fill="currentColor" stroke="#10b981" strokeWidth="2" className="text-background" />
                  <g transform="translate(130, -10)">
                     <rect width="40" height="18" rx="4" fill="white" className="dark:fill-card" />
                     <text x="20" y="12" fill="currentColor" fontSize="8" fontWeight="bold" textAnchor="middle" className="dark:fill-foreground">+{gearInUseCount}</text>
                  </g>
                </svg>
                <div className="absolute bottom-0 left-0 w-full flex justify-between text-[8px] text-muted-foreground font-medium">
          ...
          <div className="bg-card border border-border rounded-[24px] p-6">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="Search" 
                    className="bg-muted border border-border rounded-lg pl-8 pr-3 py-1.5 text-xs text-foreground focus:outline-none focus:border-muted-foreground/50"
                  />
                </div>
                <button className="flex items-center gap-1.5 bg-muted border border-border rounded-lg px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted/70">
                  Filters <ChevronDown className="w-3.5 h-3.5 text-muted-foreground" />
                </button>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[600px]">
                <thead>
                  <tr>
                    <th className="pb-3 pt-2 text-muted-foreground font-normal text-xs border-b border-border/60 w-1/3">Project</th>
                    <th className="pb-3 pt-2 text-muted-foreground font-normal text-xs border-b border-border/60 w-1/4">Assignee</th>
                    <th className="pb-3 pt-2 text-muted-foreground font-normal text-xs border-b border-border/60 w-1/4">Due date</th>
                    <th className="pb-3 pt-2 text-muted-foreground font-normal text-xs border-b border-border/60 w-1/12">Status</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    displayedPlanEvents.map((item: any, idx: number) => (
                      ...
                      >
                        <td className="py-4 border-b border-border/40 dark:border-zinc-800/40">
                          <div className="flex items-center gap-3">
                            <FolderOpen className="w-4 h-4 text-muted-foreground group-hover:text-emerald-600 transition-colors" />
                            <span className="text-sm font-medium text-foreground group-hover:text-foreground transition-colors truncate max-w-[200px] sm:max-w-[250px]">
                              {item.title}
                            </span>
                          </div>
                        </td>
                        <td className="py-4 border-b border-border/40 dark:border-zinc-800/40">
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-muted flex items-center justify-center text-[10px] font-bold border border-border text-muted-foreground">
